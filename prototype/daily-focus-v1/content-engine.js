...
(() => {
  ...
  let libraryCategory='global',libraryItemIndex=0;
  ...
  function categoryLabel(category){return category==='original'?'原创短句':category==='global'?'世界文学':'中文古典';}
  function categoryMode(category){return category==='original'?'original':category==='global'?'global':'classical';}
      ...
      <div class="modal library-modal">
        ...
        <div class="library-grid">
          ...
          <div class="library-right">
            <div><b id="libraryHeading" style="font-size:15px;font-weight:450"></b><p id="libraryHint" style="margin:5px 0 0"></p></div>
  ...
  function renderLibrary(){
    ...
    categories.innerHTML=data.map(d=>{
      const n=entriesFor(d.key).length;
      return `<button class="library-cat ${libraryCategory===d.key?'active':''}" data-cat="${d.key}"><span><b>${d.label}</b><small>${d.desc}</small></span><span class="library-count">${n} 条</span></button>`;
    }).join('');
    ...
    const heading=overlay.querySelector('#libraryHeading'),hint=overlay.querySelector('#libraryHint'),list=overlay.querySelector('#libraryList'),detail=overlay.querySelector('#libraryDetail');
    heading.textContent=`${categoryLabel(libraryCategory)} · 预览 (${entries.length} 条)`;
    hint.textContent='点击任一条目查看完整内容与来源信息。';
    list.innerHTML=entries.slice(0,libraryCategory==='original'?18:50).map((item,i)=>`<button class="library-item ${i===libraryItemIndex?'active':''}" data-index="${i}"><b>${item.meta||categoryLabel(libraryCategory)}</b><span>${item.text}</span></button>`).join('') || '<p>内容加载中。</p>';
