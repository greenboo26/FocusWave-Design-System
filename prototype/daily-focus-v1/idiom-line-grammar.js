...
window.FocusWaveIdiomGrammar = (() => {
  ...
  function petal(x,y,cx,cy,w,h,t,phase,strength){
    ...
    return {
      dx: Math.cos(a)*w*.018*envelope*petals,
      dy: Math.sin(a)*h*.030*envelope*petals
    };
  }
  ...
  function passingGlance(x,y,w,h,t,lineIndex,seed=0){
    const xn=x/w;
    const sweep=Math.exp(-Math.pow((xn-.46)/.22,2));
    let dx=w*.010*sweep*Math.sin(t*.75+lineIndex*.08);
    let dy=h*.005*sweep*Math.sin(t*.54+lineIndex*.11);
    const c1=petal(x,y,w*.58,h*.38,w,h,t,seed*.31,.82);
    const c2=petal(x,y,w*.73,h*.67,w,h,t,seed*.47+1.3,.58);
    dx+=c1.dx+c2.dx; dy+=c1.dy+c2.dy;
    return {dx,dy};
  }
  ...
  function competingCurrents(x,y,w,h,t,lineIndex,seed=0){
    ...
    let ox=0,oy=0;
    points.forEach((p,k)=>{
      const dx=(x-p.x)/w,dy=(y-p.y)/h,r2=dx*dx+dy*dy+.012;
      const e=Math.exp(-r2*19);
      const a=Math.atan2(dy,dx)+p.s*(.8+.16*Math.sin(t*.7+k));
      ox+=Math.cos(a)*w*.026*e*(.62+.22*Math.sin(lineIndex*.17+k+seed));
      oy+=Math.sin(a)*h*.038*e*(.62+.22*Math.cos(lineIndex*.13+k));
    });
    return {dx:ox,dy:oy};
  }
  ...
  function returningOne(x,y,w,h,t,lineIndex){
    const xn=x/w;
    const convergence=Math.max(0,Math.min(1,(xn-.18)/.68));
    const center=h*.5;
    const dy=(center-y)*.022*convergence;
    const residual=Math.sin(t*.22+lineIndex*.13)*h*.004*(1-convergence);
    return {dx:0,dy:dy+residual};
  }
