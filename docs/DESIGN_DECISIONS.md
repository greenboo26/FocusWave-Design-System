...
首页随机轮换海面、线香、山形三种不同构图，而不是在同一海面线场上只替换文字。长期趋势图使用专注指数、稳定片段、恢复表现三条曲线、时长背景柱、个人平均虚线、趋势阴影、逐段绘制和悬停读数。会话参数图按注意状态分段着色：稳定为苔绿、游移为蓝灰、分散为赭色、回收为青色，图线、背景区间和图例使用同一套映射。
...
枯山水默认恢复实体版本：可见沙粒、绕石纹路、石体明暗与投影；正式页直接复用 `lab/assets/karesansui/` 的木质抽屉与细耙、中耙、粗耙、平整棒资产。庭具只在接触沙面时被拿起，并随拖动方向旋转；移动、撤销和重置作为托盘控制保留，不再使用 SVG 文字工具栏替代实体庭具。

## D-023 内容规模与洞察图必须用真实数量验收

内容库的验收同时检查可见覆盖和底层条目数，不再只改分类徽标。当前批次包含 64 条人工原创、128 条离线批量新创、16 篇世界文学和 16 篇中文古典；新创内容保留内部生成与审核信息，前台使用中性类别名。

洞察范围按钮必须实际切换数据窗口。右侧模式不只显示文字结论，还应同步呈现比较柱图或适配区间曲线；动态效果在入场后保留低强度数据游标，以避免“动画结束后整页完全静止”。

## D-024 默认文字模式驱动今日页引言库，引言与概念同题匹配

设置中的「默认文字模式」消费的是今日页轮换引言的内容库，四种模式对应四个已审核的本地内容文件；专注进行中保持无文字的低干扰状态不变（与 D-020、Live Focus 的沉浸要求一致）。练习与总结中的产品文案不属于人文层，不受模式影响。

- **极简**：今日页不显示轮换引言，恢复产品默认主文案；数据场概念轮换保留。
- **原创短句**：`original-state-lines.json`（64 条人工原创）与 `generated-state-lines.json`（128 条离线批量新创）按「意象主题 × 状态」精确取用，与当前轮换概念同题；前台署名使用中性类别名「FocusWave 原创短句」，生成方式留在条目内部（D-022）。
- **世界文学**：`world-public-domain.json`（16 篇公版中译），显示中文译文与「作者 · 作品」，原文悬浮可查。
- **中文古典**：`classical-zh.json`（16 篇）与 `curated-quotes.json`（20 条审核精选），显示原文与「作者《作品》」或既有出处。

匹配规则遵循 `system/TEXT_QUOTE_ENGINE.md` 的语义匹配方向：主题与状态精确命中优先，其次同主题，最后全库回退，任何库都不因标签稀疏而空转。模式在设置中切换后通过 `focuswave:text-mode-changed` 事件即时生效，跨标签页通过 `storage` 事件同步；今日页重载内容库并原位重绘，不需要刷新。

该决策同时落实 D-023 的验收口径：64/128/16/16 的内容批次从「文件存在」变为「产品内可见、可切换、按语义寻址」。
